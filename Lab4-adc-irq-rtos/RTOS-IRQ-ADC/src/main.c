...
static void AFEC_pot_callback(void) {
  ...
  BaseType_t xHigherPriorityTaskWoken = pdTRUE;  //pq true?
...
static void task_adc(void *pvParameters) {
  ...
  while (1) {
    if (xQueueReceive(xQueueADC, &(media_movel), 1000)) {
      printf("ADC: %d \n", media_movel);
    } else {
...
static void task_proc(void *pvParameters) {
  ...
  adcData adc;
  int contador = 0;
  int soma_10_termos = 0;
  while (1) {
    if (xQueueReceive(xQueuePROC, &(adc), 1000)) {
		if (contador == 10){
			int media_movel = soma_10_termos/10;
			contador = 0;
			soma_10_termos = 0;
			BaseType_t xHigherPriorityTaskWoken = pdTRUE;  //pq true?
